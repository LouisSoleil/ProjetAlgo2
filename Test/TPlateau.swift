...
// Pre : x et y doivent correspondre aux coordonnées d'une position du plateau.
	...
	public func RegionDispo (p :TPiece , x : Int , y : Int) -> Bool {
		var check : Bool = true
		if (x == 0 || x == 1) {	//On regarde dans quelles zones on se trouve 
			if (y == 1 || y == 0){
				var i : Int = 0
				var j : Int = 0
				while (i <= 1 && check == true){
					while (j <= 1 && check == true){
						if jeu[i][j] != nil && jeu[i][j]!.forme == p.forme && jeu[i][y]!.couleur != p.couleur {
							check = false
						}
						j += 1
					}
	...
	public func LigneRemplie (x : Int , y : Int) -> Bool {
		...
		while (i < 3 && check == true){
			...
			if tableauPiece.contains (where: { el in
					jeu[i][y]!.forme == el.forme
				}) {
				check = false
			}
			i+=1
	...
	public func ColonneRemplie (x : Int , y : Int) -> Bool{
		...
		while (i < 3 && check == true){
			...
			if tableauPiece.contains (where: { el in
					jeu[i][y]!.forme == el.forme
				}) { //Si le plateau contient déjà cette forme
				check = false
			}
			i+=1
		}
